...
int main(){
	int i,u,counter=0;char state=0,*buffer,*free_buffer=malloc(sizeof(char));
	...
	while(!(state&1)){	//Loop is broken when state's 0x1 bit is set.
		...
		for(i=0;(buffer[i]!='\n')&&(buffer[i]!='\0')&&(buffer[i]!='\r');i++);buffer[i]='\0';

		//Command time!
		if((!strcmp(buffer,"exit"))||(!strcmp(buffer,"quit"))||(!strcmp(buffer,"q")))state^=1;
		...
		//List open database (commands either list or ls)
			...
			i=0;
			for(temp=database;temp!=NULL;temp=temp->next)printf("%4d: %s - %s (%d)\n",++i,temp->artist,temp->album,temp->year_i);
		}
		...
		//Add a new entry. (command add)
			...
			for(i=0;temp->artist[i]!='\n'&&temp->artist[i]!='\0'&&temp->artist[i]!='\r';i++);temp->artist[i]='\0';
			for(i=0;temp->album[i]!='\n'&&temp->album[i]!='\0'&&temp->album[i]!='\r';i++);temp->album[i]='\0';
			for(i=0;temp->year[i]!='\n'&&temp->year[i]!='\0'&&temp->year[i]!='\r';i++);temp->year[i]='\0';
		...
		//Remove item from list (command remove <index>)
			...
			if(temp){
				...
				if(*buffer=='y'||*buffer=='\n'||*buffer=='\r'||*buffer=='\0')database=rm(database,u);
...
char *desanitize(char *buffer){
	...
	for(i=0;buf[i]!='\0';i++)switch(buf[i]){
		case '_':buf[i]=' ';break;
		case '\\':buf[i]='_';break;
		case '\r':case '\n':buf[i]='\0';break;
	}
...
list sort(list db,int mode){
	...
	for(temp_l=db;temp_l!=NULL;temp_l=temp_l->next)size++;
	base->next=db;
...
char *mash_case(char *source){
	...
	for(i=0;source[i]!='\0';i++)if(source[i]>0x40 && source[i]<0x5B)value[i]=source[i]+0x20;else value[i]=source[i];
	return value;
}
...
list rm(list db,int pos){
	...
	for(temp=db;pos--&&temp!=NULL;temp=temp->next)prev=prev->next;
